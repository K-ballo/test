...
void test_entry_hash_agrees_with_name()
{
    ...
    EGGS_TEST_DETAIL_CHECK(hash(e) == hash(std::string_view{"foo"}));
}
...
void test_entry_equal_agrees_across_overloads()
{
    ...
    EGGS_TEST_DETAIL_CHECK(equal(a, b));
    EGGS_TEST_DETAIL_CHECK(equal(a, std::string_view{"foo"}));
    EGGS_TEST_DETAIL_CHECK(equal(std::string_view{"foo"}, b));
}
...
void test_entry_distinguishes_different_names()
{
    ...
    EGGS_TEST_DETAIL_CHECK(!equal(a, b));
    EGGS_TEST_DETAIL_CHECK(!equal(std::string_view{"foo"}, b));
    EGGS_TEST_DETAIL_CHECK(!equal(a, std::string_view{"bar"}));
}
...
void test_entry_heterogeneous_find()
{
    ...
    EGGS_TEST_DETAIL_CHECK(it != cases.end());
    EGGS_TEST_DETAIL_CHECK(it->name == "foo");
