...
// TEST_CASE with no parameters auto-registers under its bare name.
TEST_CASE(stateless, "zero-parameter test")
{
    CHECK(1 + 1 == 2);
}

// TEST_CASE with one parameter: REGISTER_P passes one value directly.
...
// TEST_CASE with two parameters: REGISTER_P passes two values directly.
...
// TEST_CASE with one auto parameter: type is deduced from the value passed
// by REGISTER_P.
TEST_CASE(
...
// TEST_CASE with two auto parameters: each deduces independently.
TEST_CASE(
...
// TEST_CASE with constrained parameter: enforced at registration.
TEST_CASE(
